...
The app opens directly on the forest arrival screen. The headline leads straight into the shared **Choose a writing mode** setup; there is no tutorial, preview, or added marketing copy before writing. **Download for Mac** remains a direct GitHub release link in the header, while **How it works** holds the fuller explanation. All four modes have equal, described radio choices and a selected-mode Start button. If local storage is unavailable, writing remains accessible, but preferences cannot survive a restart.
